...
public class HibernateEtudiantDaoImpl {
	...
	public List<Etudiant> finByName(String pName) {
		...
		try {
			...
			String hqlQuery = "from com.bo.Etudiant where nom like ?1 ";

			// Préparer la requete
			Query<Etudiant> query = session.createQuery(hqlQuery, Etudiant.class);
	...
	public List<Etudiant> findByCriteria(String pClassName, String[] pColName, Object[] pColVal) {
		...
		try {
			...
			for (int i = 0; i < pColName.length; i++) {
				...
				if(i==0) {
					sb.append(pColName[i] + "=?" + (i+1));	
				}
				else {
					sb.append(" and " + pColName[i] + "=?" + (i+1));
				}
